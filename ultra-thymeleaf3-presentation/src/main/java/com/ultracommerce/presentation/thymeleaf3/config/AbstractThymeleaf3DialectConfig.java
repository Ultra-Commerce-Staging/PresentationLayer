...
import com.ultracommerce.presentation.thymeleaf3.dialect.UltraThymeleaf3Dialect;
import com.ultracommerce.presentation.thymeleaf3.processor.ArbitraryHtmlInsertionProcessor;
import com.ultracommerce.presentation.thymeleaf3.processor.UltraThymeleaf3CacheProcessor;
...
public abstract class AbstractThymeleaf3DialectConfig {
    ...
    @Autowired(required = false)
    protected Set<UltraProcessor> ucProcessors = new LinkedHashSet<>();
    ...
    @Bean
    public Set<IDialect> ucEmailDialects() {
    ...
    @Bean
    public UltraThymeleaf3Dialect ucDialect() {
        UltraThymeleaf3Dialect dialect = new UltraThymeleaf3Dialect();
        Set<IProcessor> allProcessors = new LinkedHashSet<>();
        allProcessors.addAll(ucDialectProcessors());
    ...
    public Set<IProcessor> ucDialectProcessors() {
        return configUtil.getDialectProcessors(ucProcessors);
    }
    ...
    static class AbstractThymeleaf3ProcessorConfig {
        @Bean
        public IProcessor ucArbitraryHtmlInjectionProcessor() {
            return new ArbitraryHtmlInsertionProcessor();
        }

        @Bean
        public IProcessor ucCacheProcessor() {
            return new UltraThymeleaf3CacheProcessor();
        }
